...
- The bridge runtime always probes `CODEX_WS_URL` first. If a Codex app-server is already listening there, the bridge reuses it; otherwise, when `PHODEX_MANAGE_CODEX` is left enabled, the bridge falls back to launching its own background Codex process.
- The installer should warn when managed fallback remains enabled, because a background Codex process started by LaunchAgent may not share the foreground Codex app's macOS `Files & Folders`, `Full Disk Access`, or `Automation` approvals.
