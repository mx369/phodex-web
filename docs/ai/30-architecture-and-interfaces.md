...
- Newly created threads are emitted through `bridge:thread:created` with a `ThreadRecord` and explicit known-empty history (`totalMessages=0`, `isHydrating=false`). Creating a thread should not immediately trigger a full `thread/list` sync or a selected-thread history read for that empty thread, and relay/client create timeouts should leave room for `thread/start` to finish when Codex app-server is busy with another running turn.
