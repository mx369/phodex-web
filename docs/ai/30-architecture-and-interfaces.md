...
- The shell installer ensures a local Codex app-server is reachable at `ws://127.0.0.1:8765` during the same install command. It reuses an existing listener when present; otherwise, on macOS it registers a separate `com.phodex.codex.*` LaunchAgent that re-enters the login shell and execs `codex app-server --listen ...`, then writes `PHODEX_CODEX_WS_URL` plus `PHODEX_MANAGE_CODEX=false` for the bridge.
