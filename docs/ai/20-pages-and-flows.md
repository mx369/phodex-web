...
| Sidebar | Partial | Real thread list, select, rename, archive, project-targeted local/worktree create, tap-outside dismissal, and collapsible project groups; the drawer keeps search plus new-chat actions, while per-thread rows now prioritize title, time, preview, and only the most useful lightweight badges so more conversation rows stay visible | Final parity and a few density tradeoffs still differ |
