...
| New chat -> first send | Real | Dialog-based create flows remain in the drawer, and the current-thread overflow menu can also start a fresh local or worktree chat without reintroducing a dedicated toolbar row; local pending thread state stays visible after the bridge accepts the request and only resolves or fails on the explicit create result from Codex |
