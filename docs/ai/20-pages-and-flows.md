...
| Connected Home -> add another Mac | Real | Home keeps the install card visible after a bridge connects, can mint a fresh command with `New command`, and now shows all registered Macs so a second-machine install is visible immediately without replacing the first card |
