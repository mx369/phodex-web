...
| Home empty | Partial | Real bridge/account state drives the full-page mobile shell; the post-login install card now stays available even when one Mac is already connected so the same account can mint a fresh command for another device, and Home now renders a deduped per-Mac device list instead of piling up stale install records for the same hostname. Only `connected` device cards are switchable: the active card opens chats, another ready Mac switches the active bridge, and `linking` or offline cards stay inert. | Final spacing and device-list parity still differ |
| Sidebar | Partial | Real thread list, select, rename, archive, project-targeted local/worktree create, tap-outside dismissal, and collapsible project groups; the drawer now uses a compact action strip plus condensed footer controls so more conversation rows stay visible | Final parity and a few density tradeoffs still differ |
| Turn empty | Partial | Real selected thread, blank conversation canvas, bottom workspace-action rail, composer, pending-thread state, a current-project `New Chat` shortcut from the thread toolbar, a persisted `Normal / Fast` runtime toggle, and single-image attachment send | Structured-input replacement and final polish are still missing |
| Turn with messages | Partial | Real streaming, richer tool/file/system cards, latest-jump, an inline pending-send that now stays visible until the real user message lands, queued drafts with inline waiting previews plus bottom-card controls, pinned plan, thread-scoped project diff/file inspectors, compact inline file-reference chips for Markdown local file links, and inline rendering of user-sent images | Some empty branches, queue states, and toolbar/sheet parity still differ |
...
| New chat -> first send | Real | The thread-toolbar `New Chat` now starts a fresh chat in the current project immediately, while dialog-based create flows remain in the drawer; local pending thread state stays visible instead of dropping to a blank canvas before the real Codex thread resolves |
...
| Queue while running | Real | Stores the draft locally, preserves its runtime settings, inserts the follow-up into the inline waiting bubble plus the bottom queued-draft controls immediately on tap, and auto-drains the oldest queued draft after the active run completes without stealing the user back to that thread if they already switched elsewhere |
