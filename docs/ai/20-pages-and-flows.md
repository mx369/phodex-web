...
| Home empty | Partial | Real bridge/account state drives the full-page mobile shell; the post-login install card stays available even when one bridge device is already connected, defaults to a compact platform switch + copy + reveal control, and Home renders a deduped per-device list instead of piling up stale install records for the same hostname. Only `connected` device cards are switchable: the active card opens chats, another ready device switches the active bridge, and `linking` or offline cards stay inert. | Final spacing and device-list parity still differ |
