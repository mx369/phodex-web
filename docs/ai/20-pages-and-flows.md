...
| Home empty | Partial | Real bridge/account state drives the full-page mobile shell; the post-login install card now stays available even when one Mac is already connected so the same account can mint a fresh command for another device, and Home now renders every registered bridge device with `Active` / `Saved` state chips instead of collapsing to one Mac card. Online device cards are tappable: the active card opens chats, another online card switches the active bridge, and offline cards stay disabled. | Final spacing and device-list parity still differ |
...
| Connected Home -> add another Mac | Real | Home keeps the install card visible after a bridge connects, can mint a fresh command with `New command`, now shows all registered Macs so a second-machine install is visible immediately without replacing the first card, and lets the user tap an online device card to make it active |
