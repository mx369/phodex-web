...
| Queue while running | Real | Stores the draft locally, preserves its runtime settings, and now inserts the follow-up into the inline waiting bubble plus the bottom queued-draft controls immediately on tap instead of waiting for the relay echo; the oldest queued draft still auto-drains after the active run completes |
