...
| Sidebar Drawer | authenticated session after the drawer parity, UI-standards, typography-tightening, and brandless-shell passes | drawer render, SVG icon system, explicit create sheet, project-group `+` preselection, folder-name and absolute-path cwd creation, real worktree start path, row rename action, connection footer, settings/archive/disconnect actions, long-list create-sheet overflow handling, normalized shared button sizing/radius inside the drawer surfaces, reduced secondary text prominence inside drawer cards/metadata, a smaller create-sheet guidance scale, and a header that no longer prints the brand name | pass | `.artifacts/qa-76-drawer-open.png`, `.artifacts/qa-77-drawer-create-sheet.png`, `.artifacts/qa-78-drawer-worktree-created.png`, `.artifacts/qa-80-drawer-rename-fixed.png`, `.artifacts/qa-81-drawer-custom-folder-input.png`, `.artifacts/qa-82-drawer-custom-absolute-path.png`, `.artifacts/qa-cdp/qa-86-drawer-create-sheet-scrollable.png`, `.artifacts/qa-cdp/qa-88-drawer-ui-standards.png`, `.artifacts/qa-cdp/qa-89-create-sheet-ui-standards.png`, `.artifacts/qa-cdp/qa-96-drawer-ui-tightened-2.png`, `.artifacts/qa-cdp/qa-101-create-sheet-typography-tightened.png`, `.artifacts/qa-cdp/qa-110-drawer-brandless.png` |
| Turn View | authenticated existing threads after richer-thread-surface, work-state, scroll-state, composer-polish, UI-standards, collapsed-composer baseline, send-state, send-button redo, and diff-history fallback passes | turn toolbar chips, real command/file/tool/subagent activity cards, queued-draft work-state band, pinned-plan surface, internal timeline scrolling, visible latest-jump button, return-to-bottom behavior, a custom model picker instead of the native browser dropdown, normalized send/stop/latest-jump control sizes in the mobile composer, a compact two-row collapsed composer, a neutral empty-state send control that only promotes into the orange ready state once text is present, a screenshot-compared send control that is inset from the right edge and aligned with the model-picker centerline, reduced secondary copy density around the composer/footer surfaces, and a completed temp-repo diff pass that keeps `+1 -0` in the toolbar plus `README.md` file-change rows after sync | pass | `.artifacts/qa-64-richer-thread-surfaces-viewport.png`, `.artifacts/qa-65-richer-thread-surfaces-bottom-viewport.png`, `.artifacts/qa-68-work-state-band.png`, `.artifacts/qa-69-queued-draft-band-bottom.png`, `.artifacts/qa-70-plan-band-bottom.png`, `.artifacts/qa-73-turn-scroll-bottom.png`, `.artifacts/qa-cdp/qa-87-turn-scroll-black-centered.png`, `.artifacts/qa-cdp/qa-90-thread-controls-ui-standards.png`, `.artifacts/qa-cdp/qa-91-latest-jump-ui-standards.png`, `.artifacts/qa-cdp/qa-93-thread-ui-tightened.png`, `.artifacts/qa-cdp/qa-95-thread-ui-tightened-2.png`, `.artifacts/qa-cdp/qa-97-thread-bottom-send-tightened.png`, `.artifacts/qa-cdp/qa-99-turn-send-baseline-tightened.png`, `.artifacts/qa-cdp/qa-107-send-button-empty-disabled.png`, `.artifacts/qa-cdp/qa-108-send-button-active-ready.png`, `.artifacts/qa-cdp/qa-118-send-button-before-redo.png`, `.artifacts/qa-cdp/qa-120-send-button-after-redo-2.png`, `.artifacts/qa-cdp/qa-121-send-button-ready-redo.png`, `.artifacts/qa-cdp/qa-83-model-picker-open.png`, `.artifacts/qa-cdp/qa-84-model-picker-mini-selected.png`, `.artifacts/qa-cdp/qa-116-diff-thread-surface-restored.png`, `.artifacts/qa-cdp/qa-117-drawer-thread-diff-meta-restored.png` |
...
- Shell/navigation pass: home, sidebar, about, paywall, and archived pages were rerun after the mobile-page refactor, with home/sidebar rerun again after the latest source-parity pass.
...
- Send-state pass: the active thread was rerun after removing the false-positive orange send state from empty composers; the send control now stays neutral until the user has meaningful text.
- Send-button redo pass: the same active thread was rerun after comparing the real composer screenshot against the runtime UI and rebuilding the empty-state send control so it sits inset from the right edge, aligns with the model-picker centerline, and keeps the same compact `30px` footprint in both idle and ready states.
- Diff-history fallback pass: a fresh temp git repo was created, a real one-line `README.md` edit was run through Codex, and the synced thread was rechecked for all three visible diff surfaces: toolbar `+1 -0`, drawer thread metadata `+1 -0`, and a persistent `README.md` file-change row after completion.
- Brandless-shell pass: the authenticated main shell was rerun after removing redundant brand text from the topbar and drawer header, leaving only the page/thread title and app logo.
